...
def thread_detail(request, id):
    ...
    if request.method == 'POST':
        form = PostForm(request.POST, request.FILES)  # ✅ Поддержка загрузки файлов
        if form.is_valid():
...
def post_edit(request, id):
    ...
    if request.method == "POST":
        form = PostForm(request.POST, request.FILES, instance=post)  # ✅ Поддержка загрузки файлов
        if form.is_valid():
            form.save()
...
def post_create(request, thread_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            post = form.save(commit=False)
            post.thread = thread  # Привязываем пост к теме
            post.save()
            return redirect('thread_detail', id=thread_id)  # Перенаправляем обратно в тему
    else:
        form = PostForm()
